...
## 3. Menu de Módulos (Launcher)

Para facilitar a navegação e o gerenciamento de tarefas, o sistema conta com um menu centralizado.

1.  No menu lateral, clique em **Menu de Módulos**.
2.  Você verá ícones grandes organizados por área (Qualidade, Legalização, Auditoria, Segurança).
3.  **Badges de Pendência:** Fique atento aos contadores (números vermelhos ou amarelos) sobre os ícones. Eles indicam itens que exigem sua atenção imediata, como:
    *   Contratos aguardando assinatura.
    *   Documentos vencidos ou próximos do vencimento.
    *   Ocorrências externas não tratadas.
    *   Auditorias em andamento.

---

## 4. Módulo de Qualidade (Ocorrências)
...
Para localizar rapidamente um documento sem precisar entrar em cada unidade:
1.  Acesse o **Menu de Módulos**.
2.  No bloco de Legalização, clique em **Buscar Documentos**.
3.  Utilize a barra de pesquisa para encontrar por nome do documento, unidade, área ou palavra-chave (Tag).
4.  Use o filtro de **Status** para ver apenas os Vencidos, Vigentes ou Pendentes.
5.  A lista mostrará os resultados de todas as unidades que você tem permissão de acesso.
...
## 6. Gestão de Contratos e Aditivos
...
Se você precisa encontrar um contrato específico mas não lembra a unidade:
1.  Acesse o **Menu de Módulos**.
2.  No bloco de Legalização, clique em **Buscar Contratos**.
3.  Digite o número do contrato, nome do fornecedor ou parte do objeto.
4.  O sistema listará todos os contratos correspondentes em toda a rede.
...
## 7. Módulo de Auditoria Interna

O módulo de Auditoria permite planejar, executar e relatar auditorias de qualidade baseadas em normas (ISO, ONA, PALC, etc.).

*   **Cronograma:** Visualize todas as auditorias em um calendário interativo.
*   **Checklists:** Responda aos requisitos da norma diretamente no sistema.
*   **Apontamentos:** Registre Não Conformidades e Oportunidades de Melhoria.
*   **Relatório:** Gere o relatório final em PDF automaticamente.

> **Guia Completo:** Para um treinamento aprofundado sobre como conduzir uma auditoria utilizando o sistema, consulte o **[Manual Específico de Auditoria](MANUAL_AUDITORIA.md)**.
